          ...
          <form onSubmit={handleSubmit} className="space-y-4 text-xs font-semibold">
            ...
            <div className="flex items-center justify-end gap-3 pt-4 border-t border-zinc-100">
              <button
                type="button"
                onClick={onClose}
                className="px-4 py-2.5 rounded-xl border border-zinc-200 text-zinc-700 hover:bg-zinc-100 font-bold transition-all"
              >
                Cancel
              </button>
              <button
                type="submit"
                disabled={isSaving}
                className="px-5 py-2.5 rounded-xl bg-zinc-950 hover:bg-black text-white font-bold transition-all shadow-xs disabled:opacity-50"
              >
                {isSaving ? "Saving..." : isEditing ? "Update Entry" : "Record Entry"}
              </button>
            </div>
          </form>
        </motion.div>
